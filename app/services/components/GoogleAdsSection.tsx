...
export default function GoogleAdsSection({ content }: GoogleAdsSectionProps) {
  ...
  // Since we know content.campaignTypes will always have at least one item in a real scenario,
  // we'll use a type assertion to tell TypeScript this is safe
  const defaultCampaign = content.campaignTypes[0];
  
  // Get the active campaign type data with fallback to defaultCampaign
  // Using type assertion to tell TypeScript this will never be undefined
  const currentCampaign = (content.campaignTypes.find(
    (campaign) => campaign.id === activeCampaignType
  ) || defaultCampaign) as (typeof content.campaignTypes)[0];
        ...
        <div id="google-ads-funnel-wrapper" className="animate-element">
          ...
          >
            <div id="google-ads-funnel-header" className="mb-6 flex justify-between items-center">
              <h3 id="google-ads-funnel-title" className="text-xl md:text-2xl font-bold text-[var(--text-100)]">
            ...
            <div id="google-ads-funnel-steps" className="relative">
              ...
              <div id="google-ads-funnel-shape" className="absolute inset-0 z-0">
                <div className="h-full w-full max-w-4xl mx-auto flex flex-col">
                  <div className="h-1/3 bg-gradient-to-b from-[var(--bg-200)] to-[var(--bg-300)] opacity-10 rounded-t-xl"></div>
                  <div className="h-1/3 bg-gradient-to-b from-[var(--bg-300)] to-[var(--accent-green-base)] opacity-10"></div>
                  <div className="h-1/3 bg-gradient-to-b from-[var(--accent-green-base)] to-[var(--accent-green)] opacity-10 rounded-b-xl"></div>
                </div>
              </div>
              ...
              <div id="google-ads-metrics-grid" className="relative z-10 grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                {currentCampaign.metrics.map((metric) => (
                  <div
                    ...
                    className="metric-chip bg-[var(--bg-100)] p-4 rounded-xl border border-[var(--bg-300)] shadow-sm flex flex-col items-center text-center"
                  ...
                  >
                    <div id={`google-ads-metric-${metric.id}-value-container`} className="mb-1">
                      <span
                        ...
                        className="text-2xl md:text-3xl font-bold text-[var(--text-100)]"
                      >
                    ...
                    <div id={`google-ads-metric-${metric.id}-label-container`}>
                      <span
                        ...
                        className="text-xs md:text-sm text-[var(--text-200)] font-medium uppercase tracking-wide"
                      >
                        {metric.label}
                      </span>
                    </div>
