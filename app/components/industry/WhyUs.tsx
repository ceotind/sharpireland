...
export default function WhyUs() {
  ...
  const renderIcon = (Icon: React.ElementType) => {
    return (
      <div className="text-[var(--accent-green)]" style={{ fontSize: 28 }}>
        {Icon && <Icon />}
      </div>
    );
  };
      ...
      <div className="w-full max-w-3xl mx-auto px-4">
        <div className="text-center mb-14">
          ...
          <h2 className="animate-element mt-4 text-3xl sm:text-4xl font-bold text-[var(--text-100)]">
            {whyUs?.heading} <span className="text-[var(--accent-green)]">{whyUs?.headingHighlight}</span>
          </h2>
                ...
                <li key={idx} className="animate-element flex items-start relative">
                  ...
                  <div className="flex-1 pl-4">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1">
                      ...
                      <span className="self-start sm:self-auto px-2 py-0.5 rounded-full bg-[var(--bg-200)] text-[var(--accent-green)] text-xs font-medium">{reason.stat}</span>
                    </div>
